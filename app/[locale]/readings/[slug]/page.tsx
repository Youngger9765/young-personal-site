...
import { getReadingDigest, getReadingDigests } from "@/lib/readings";
...
export async function generateStaticParams() {
  ...
  for (const locale of locales) {
    ...
    for (const digest of digests) {
      params.push({ slug: digest.slug, locale });
    }
  }
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: {
        'zh-TW': `${SITE_URL}/zh-TW/readings/${slug}`,
        en: `${SITE_URL}/en/readings/${slug}`,
      },
    },
  };
}

export default async function ReadingDigestPage({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  const { slug, locale } = await params;
  const digest = await getReadingDigest(slug, locale);
  ...
  const formattedDate = new Date(digest.date).toLocaleDateString(
    locale === 'zh-TW' ? 'zh-TW' : 'en-US',
    { year: 'numeric', month: 'long', day: 'numeric' }
  );

  const articleUrl = `${SITE_URL}/${locale}/readings/${slug}`;
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <header className="max-w-2xl mx-auto px-6 pt-32 pb-8">
        {/* Week badge + stats */}
        <div className="flex items-center gap-3 mb-6 text-sm text-stone-400">
          <span className="inline-block px-3 py-1 bg-accent/10 text-accent font-bold text-sm rounded-lg uppercase tracking-wide">
            {digest.week.replace('2026-', '')}
          </span>
          {digest.articleCount && digest.sourceCount && (
            <span>{digest.articleCount} {t('readings.articles')} | {digest.sourceCount}+ {t('readings.sources')}</span>
          )}
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 pb-8">
        <div className="flex items-center gap-3 py-6 border-y border-stone-200">
          ...
          {/* X (Twitter) share */}
          <a
            href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(articleUrl)}&text=${encodeURIComponent(digest.title)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-stone-200 text-stone-600 hover:border-accent hover:text-accent transition-colors text-sm"
          >
